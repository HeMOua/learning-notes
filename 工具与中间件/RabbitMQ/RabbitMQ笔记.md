...
实际开发中，生产者发送消息是毫不费力的，二消费者要与实际业务相结合，处理的时间相对较长，两者不匹配，所以就出来工作队列，可以实现1对多。
...
要点就是：发送者要**修改频道的模式**，接收者除了要修改频道外，再收到处理完消息后要**发送ack数据**，并且监听方法中要**关闭自动应答**
