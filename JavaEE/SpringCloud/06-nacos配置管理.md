...
![image-20220115182335126](img/image-20220115182335126.png)

![image-20220115183150618](img/image-20220115183150618.png)
...
测试时可以使用`@Value`注解的方法读取nacos上进行的配置
...
public class UserController {
    ...
    public String now() {
        return LocalDate.now().format(
            DateTimeFormatter.ofPattern(pattern, Locale.CHINA));
    }
```
...
方式1） 在`@Value`注入的变量所在类上添加注解`@RefreshScope`
...
这种方法适合于使用了`@Value`注入的方式

方式2）使用`@ConfigurationProperties`注入变量
...
使用`@ConfigurationProperties`进行注入的变量可以直接实现热更新
...
首先新建一个数据库，命名为`nacos`，而后导入下面的SQL：
